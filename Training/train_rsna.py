...
# Configuration
class Config:
    TRAIN_DATA_DIR = "D:/rsna_data/train_images"  # RSNA training images
    TEST_DATA_DIR = "D:/rsna_data/test_images"  # Separate test folder
    CSV_PATH = "D:/rsna_data/labels/stage_2_train_labels.csv"  # Labels for training data
    IMAGE_SIZE = 224
    BATCH_SIZE = 64
    NUM_CLASSES = 2
    EPOCHS = 20
    ...
    TRAIN_SPLIT = 0.8
    VAL_SPLIT = 0.2  # No test split here since we have a separate test folder
...
# Test Dataset Class (no labels provided)
...
def main():
    ...
    # Convert Subset objects back to DataFrames using indices
    ...
    for epoch in range(Config.EPOCHS):
        ...
        if val_metrics['f1'] > best_val_f1:
            ...
            best_epoch = epoch
            torch.save(model.state_dict(), "Training/best_rsna_model.pth")
    ...
    model.load_state_dict(torch.load("Training/best_rsna_model.pth"))
    ...
    # Save results
    with open('rsna_model_results.txt', 'w') as f:
    ...
    print("\nTraining completed! Results saved to rsna_model_results.txt")
    print("Model saved to Training/best_rsna_model.pth")
