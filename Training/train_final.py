...
class Config:
    ...
    IMAGE_SIZE = 224
    BATCH_SIZE = 32  # Updated to best value
    NUM_CLASSES = 2
    EPOCHS = 20
    BASE_LR = 0.0001  # Updated to best value
    WEIGHT_DECAY = 0.0001
    DROPOUT = 0.5  # Updated to best value
    USE_AMP = True
    PATIENCE = 5
    MIN_DELTA = 0.001
    CLASS_WEIGHTS = torch.tensor([1.0, 1.5], device=torch.device("cuda" if torch.cuda.is_available() else "cpu"))  # Updated to best value
...
def main():
    ...
    for epoch in range(Config.EPOCHS):
        ...
        if val_metrics['f1'] > best_val_f1:
            ...
            best_epoch = epoch
            torch.save(model.state_dict(), "Training/best_model_final.pth")
    ...
    model.load_state_dict(torch.load("Training/best_model_final.pth"))
    ...
    print("\nTraining completed! Results saved to model_results_final.txt")
